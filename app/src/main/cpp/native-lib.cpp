...
//    avformat_open_input(&avFmtCtx, path, NULL, &opts);
    ...
    if ((ret = avformat_open_input(&avFmtCtx, path, NULL, &opts)) != 0) {
        LOGI("%s", path, " %d", ret);
        LOGI(" %d", ret);
        return;
    } else {
...
Java_com_albert_myffmpegplayer_FFmpegAudioPlayer_sound(JNIEnv *env, jobject instance,
    ...
    avcodec_open2(codecCtx, dec, NULL);

    //转换器上下文
    SwrContext *swrContext = swr_alloc();
    //输入固定三个参数，采样率，采样位数，通道数
    //输入采样格式，采样位数
    ...
    //设置音频缓冲区间 16bit   44100  PCM数据 双通道
